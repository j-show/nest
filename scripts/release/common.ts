...
export async function getPackagePublishInfo(options: GetPackagePublishInfoOptions) {
  ...
  const currentCommitTags = new Map(
    currentTaggedCommit ? currentTaggedCommit.tags.map((x) => [x.packageName, x.version]) : [],
  );
  ...
  await Promise.all(
    Array.from(lastPackageCommit).map(async ([folderName, info]) => {
...
type YarnPackageInfoOutput<T> = YarnPackageInfoSuccessOutput<T> | YarnPackageInfoErrorOutput;
...
export async function getPackageYarnInfo<K extends keyof YarnPackageInfoResult>(
  packageName: string,
  field: K,
): Promise<YarnPackageInfoResult[K]>;
export async function getPackageYarnInfo<K extends keyof YarnPackageInfoResult>(packageName: string, field?: K) {
  const { code, stdout, stderr } = await execSafe(`yarn info ${packageName} --json ${field || ''}`, { silent: true })
    .promise;
  if (code !== 0) throw stderr;
  const text = stderr || stdout;
  try {
    const json: YarnPackageInfoOutput<YarnPackageInfoResult | ValuesOf<YarnPackageInfoResult>> = JSON.parse(
      text.trim(),
    );
    if (json.type === 'inspect') return json.data;
    return [];
  } catch (e) {
    console.info({ cmd: `yarn info ${packageName} --json ${field || ''}`, text });
    throw e;
  }
}
