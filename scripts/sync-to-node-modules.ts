...
import { packagesFullPath, readPackageJson, scope } from './build';
import { execSafe, exists, mkdirp, run } from './helper';
...
async function main(options: MainOptions) {
  ...
  const scopeFullPath = path.resolve(nodeModulesFullPath, scope);
  {
    ...
    if (!stat || !stat.isDirectory()) {
      throw new Error(
        `${JSON.stringify(root)} is not a directory (resolved to ${JSON.stringify(nodeModulesFullPath)})`,
      );
    }
  }
  {
    ...
    if (!stat || !stat.isDirectory()) {
      throw new Error(
        `scope ${JSON.stringify(scope)} is not found in node_modules (resolved to ${JSON.stringify(scopeFullPath)})`,
      );
    }
  }
  ...
  const [packageNames, targetPackageNames] = await Promise.all(
    [packagesFullPath, scopeFullPath].map(async (fullPath) => {
  ...
  await Promise.all(
    commonPackageNames.map(async (packageName) => {
      ...
      if (!(await exists(packageJsonPath))) {
        return console.info(`file ${JSON.stringify(`packages/${packageName}/package.json`)} is not found, skipped`);
      }

      const targetPackageFullPath = path.join(scopeFullPath, packageName);
      const [sourceFileNames, targetFileNames] = await Promise.all(
        [packageFullPath, targetPackageFullPath].map((fullPath) => fs.promises.readdir(fullPath)),
      );
      ...
      await Promise.all(
        Array.from(unionFiles).map(async (fileName) => {
          ...
          if (targetStat && targetStat.isFile()) {
            ...
            await fs.promises.copyFile(sourceFullPath, targetFullPath, fs.constants.COPYFILE_FICLONE);
            if (fileName === 'package.json') {
              console.info(chalk.cyan(`${packageName}/package.json has changed`));
            }
          } else {
            if (await exists(sourceFullPath)) {
              const [sourceFiles, targetFiles] = await Promise.all(
                [sourceFullPath, targetFullPath].map(async (fileFullPath) => {
                  if (await exists(fileFullPath)) {
                    return readdirAsync(fileFullPath, { deep: true, filter: (stats) => stats.isFile() });
                  }
                  return [];
                }),
              );
              ...
              if (addedFiles.length) {
                await Promise.all(
                  addedFiles.map(async (file) => {
                    ...
                    await fs.promises.copyFile(sourceFileFullPath, targetFileFullPath, fs.constants.COPYFILE_FICLONE);
                  }),
                );
              }
